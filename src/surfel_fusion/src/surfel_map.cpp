...
inactive_pointcloud(new PointCloud)
{
    ...
    // ros publisher
    dense_pointcloud_publish = nh.advertise<PointCloud>("dense_pointcloud", 10); //for 发布对每个面元扩展为6个点之后的所有点云
    pointcloud_publish = nh.advertise<PointCloud>("pointcloud", 10);            //同时发布活跃和不活跃的点云
    raw_pointcloud_publish = nh.advertise<PointCloud>("raw_pointcloud", 10);    //根据灰度图和深度图点云拼接
...
{
    string save_PLY_name = save_map_input->data + "_mesh.PLY";
    string save_dense_PCD = save_map_input->data + "dense.PCD";
    printf("save mesh modelt to %s.\n", save_PLY_name.c_str());
    printf("save dense PCD modelt to %s.\n", save_dense_PCD.c_str());
    save_mesh(save_PLY_name,save_dense_PCD);
    printf("save done!\n");
}
...
void SurfelMap::image_input(const sensor_msgs::ImageConstPtr &image_input)
{
    ...
    //printf("image %f!\n", image_ptr->header.stamp.toSec());
    synchronize_msgs();

    currtimecount = ros::Time::now();   //by hulu
}
...
void SurfelMap::depth_input(const sensor_msgs::ImageConstPtr &depth_input)
{
    ...
    //深度与彩色同步 30hz
    //ros::Time stamp = image_ptr->header.stamp;
    //由于彩色与深度不同步，且彩色时间戳早于深度，实际测得深度与彩色之间的时间差为0.033440s,
    //将深度时间减去最新的彩色图时间，获得最新的彩色与深度之间的时间差用于同步，即彩色与深度在时间上对齐
    double image_depth_dt = image_ptr->header.stamp.toSec() - image_buffer.back().first.toSec();  
    ros::Time stamp(image_ptr->header.stamp.toSec() - image_depth_dt);
    depth_buffer.push_back(std::make_pair(stamp, image));

    /*printf("image_depth_dt %f!\n", image_depth_dt);
    printf("depth_image %f!\n", image_ptr->header.stamp.toSec());
    printf("depth_synchronize %f!\n", stamp.toSec());*/
    synchronize_msgs();
}

//by hulu add for synchronize_msgs
void SurfelMap::left_image_sub(const sensor_msgs::ImageConstPtr &img_msg)
{
    m_buf.lock();
    leftimg_buf.push(img_msg);
    m_buf.unlock();
}

void SurfelMap::depth_sub(const sensor_msgs::ImageConstPtr &depth_msg)
{
    m_buf.lock();
...
cv::Mat SurfelMap::getImageFromMsg(const sensor_msgs::ImageConstPtr &img_msg)
{
    cv_bridge::CvImageConstPtr ptr;
    if (img_msg->encoding == "32FC1")
      ptr = cv_bridge::toCvCopy(img_msg, sensor_msgs::image_encodings::TYPE_32FC1);
    else
    ...
    while(1)
    {
      ...
      if (!leftimg_buf.empty() && !depth_buf.empty())
      {
        ...
        else if(time0 > time1 + 0.003)
        {
            ...
            leftimg_buf.pop();
            printf("throw leftimg\n");
        }
        else
        {
            stamp = leftimg_buf.front()->header.stamp;
            left_image = getImageFromMsg(leftimg_buf.front());
            leftimg_buf.pop();
            depth_image = getImageFromMsg(depth_buf.front());
            depth_buf.pop();
            //printf("synchronize_msgs!\n");
      ...
      if(!left_image.empty())
      {
        image_buffer.push_back(std::make_pair(stamp, left_image));
        depth_buffer.push_back(std::make_pair(stamp, depth_image));
        synchronize_msgs();
      }
...
void SurfelMap::image_depth_input(const sensor_msgs::ImageConstPtr& image_input,const sensor_msgs::ImageConstPtr& depth_input)
{
    printf("receive image!\n");
    cv_bridge::CvImagePtr image_ptr = cv_bridge::toCvCopy(image_input, sensor_msgs::image_encodings::MONO8);
    cv::Mat image = image_ptr->image;
    ...
    cv_bridge::CvImagePtr depth_ptr = cv_bridge::toCvCopy(depth_input, sensor_msgs::image_encodings::TYPE_32FC1);
    cv::Mat depth_image = depth_ptr->image;
    ros::Time depth_stamp = depth_ptr->header.stamp;
    depth_buffer.push_back(std::make_pair(depth_stamp, depth_image));
    synchronize_msgs();
...
void SurfelMap::synchronize_msgs()
{
    ...
    //printf("left_image %f!\n",image_buffer.front().first.toSec());
    //printf("depth_image %f!\n",depth_buffer.front().first.toSec());
    //printf("fuse_stamp %f!\n",pose_reference_time);
    //printf("\n");
    ...
    while(depth_buffer.size() > 0)
    {
        ...
        //printf("depth_image %f!\n",depth_buffer.front().first.toSec());  
        if(depth_time < pose_reference_time)     //如果depth_time > pose_reference_time,则会在这儿死循环
        {
    ...
    // if(poses_database.size()%2==0)
    {
        ...
        // 发布稠密点云，太耗时
        //pub_dense_pointcloud(fuse_stamp);
    }
    end_time = std::chrono::system_clock::now();
    total_time = end_time - start_time;
    ...
    const nav_msgs::OdometryConstPtr &this_pose_input)
{
    ...
    // transform the kitti pose  由于接收到的orb_slam2位姿已经变换到相对于车体坐标系下,不需要在变换
    static Eigen::Matrix4d transform_kitti = Eigen::Matrix4d::Identity();   
    /*{
        ...
        if(poses_database.size() == 0)
        {
            ...
            idea_pose(2,1) = -1.0;
            idea_pose(3,3) = 1.0;   
            transform_kitti = idea_pose * received_psoe.inverse();  //将姿态绕X轴旋转90
        }
...
void SurfelMap::publish_camera_position(ros::Time pub_stamp, geometry_msgs::Pose &pose)
{
    ...
    current_frame_visual.setScale(0.2);
    current_frame_visual.setLineWidth(0.05);
...
void SurfelMap::publish_pose_graph(ros::Time pub_stamp, int reference_index)
{
    ...
    driftfree_marker.scale.x = 0.1;   // by hulu fix  old 1.1
    driftfree_marker.scale.y = 0.1;
    driftfree_marker.scale.z = 0.1;
    ...
    drift_marker.scale.x = 0.1;     // by hulu fix  old 1.1
    drift_marker.scale.y = 0.1;
    drift_marker.scale.z = 0.1;
...
// 该函数发布的点云为 根据深度图和灰度图获得每个像素点的相对于相机的空间位置，并变换到世界坐标系下，即点云拼接
...
//将一个面元对象转换成ply文件的节点信息  将一个面元扩展为6个节点
...
//用于保存ply文件中的点为点云
void SurfelMap::save_densecloud(string save_path_name,vector<float> &vertexs,size_t numPoints)
{
    printf("saving dense pointcloud ...\n");
    PointCloud::Ptr pointcloud(new PointCloud);

    for(int i = 0; i < numPoints; i++)    //将每个点的属性写到ply文件并换行
    {
        PointType p;
        p.x = vertexs[i*6+0];        //每个点的x
        p.y = vertexs[i*6+1];        //每个点的y
        p.z = vertexs[i*6+2];        //每个点的z
        p.intensity = vertexs[i*6+3]; //每个点的color
        pointcloud->push_back(p);
    }
    // pcl::io::savePLYFile(save_path_name.c_str(), *pointcloud);
    pcl::io::savePCDFile(save_path_name.c_str(), *pointcloud);
    printf("saving dense pointcloud done!\n");
}

void SurfelMap::save_mesh(string save_path_name, string save_path_dense_PCD)
{
    printf("saving meshPLY start!\n");
    ...
    size_t numPoints = vertexs.size()/6;      //6为一个点的6个属性 x,y,z red,green,blue
    size_t numSurfels = numPoints/6;          //6为一个面元包含的点数
    stream << "ply" << std::endl;
    ...
    stream << "element face " << numSurfels * 4 <<  std::endl;    //1个面元包含4个小面,由4个三角形组成
    ...
    for(int i = 0; i < numSurfels; i++)
    {
        ...
        p5 = i * 6 + 4;
        p6 = i * 6 + 5;
        stream << "3 " << p1 << " " << p2 << " " << p3 << std::endl;    //三角形的面，一个面元有4个三角形
        stream << "3 " << p2 << " " << p4 << " " << p3 << std::endl;
        stream << "3 " << p3 << " " << p4 << " " << p5 << std::endl;
        stream << "3 " << p5 << " " << p4 << " " << p6 << std::endl;
    }
    printf("saving meshPLY done!\n");
    stream.close();

    //保存稠密点云
    save_densecloud(save_path_dense_PCD, vertexs, numPoints);
}
...
// 用于发布活跃和不活跃的点云，适用于大环境下点云发布 该点云的每个点代表一个面元，较稀疏
...
// by hulu add 将所有面元扩展为6个点，在发布，该点云较稠密
void SurfelMap::pub_dense_pointcloud(ros::Time pub_stamp)
{
    printf("pub dense ponitcloud!!\n");
    std::chrono::time_point<std::chrono::system_clock> start_time, end_time;
    std::chrono::duration<double> total_time;
    start_time = std::chrono::system_clock::now();
    ...
    end_time = std::chrono::system_clock::now();
    total_time = end_time - start_time;
    printf("construct dense point cloud cost %f ms.\n", total_time.count()*1000.0);

    start_time = std::chrono::system_clock::now();

    pointcloud->header.frame_id = "world";
    pcl_conversions::toPCL(pub_stamp, pointcloud->header.stamp);
    dense_pointcloud_publish.publish(pointcloud);
    printf("publish dense point cloud with %d points.\n", pointcloud->size());

    end_time = std::chrono::system_clock::now();
    total_time = end_time - start_time;
    printf("publish dense point cloud cost %f ms.\n", total_time.count()*1000.0);
}
